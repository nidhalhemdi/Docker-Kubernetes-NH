# Defining a Persistent Volume (PV) — Simplified Notes
...
In this lecture, we define one using the **hostPath** type — only suitable for minikube (single node), but the concepts apply to all PV types.
...
# 📌 2. Basic Structure of a Persistent Volume

Here’s the core skeleton:
...
* `apiVersion: v1` → PVs belong to the core API.
* `kind: PersistentVolume` → must be spelled exactly like this.
* `metadata.name` → your chosen PV name.

---

# 📌 3. PV Spec — Main Configuration
...
In real clusters, you’d use AWS EBS, Azure Disk, NFS, CSI, etc.
...
1. **Filesystem** → typical folder-based storage
2. **Block** → raw block device
...
Here’s what the full PV YAML looks like (conceptually):
...
This creates a standalone persistent storage resource in the cluster — *not tied to any Pod*.

---

# 🧠 Key Takeaways

### ✔ Persistent Volumes are cluster-level storage
...
### ✔ PVs can be used by any Pod

But Pods don’t use PVs directly — they use **Persistent Volume Claims (PVCs)**.
