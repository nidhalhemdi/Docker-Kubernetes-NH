...
Before creating the volume, we forces the container to **crash** using a new `/error` route:
...
* But because **no volume is used**, the container’s internal filesystem resets → **stored data disappears**.
...
* No configuration is needed → `{}` is enough

This solves the “crash and lose data” problem.
...
### 1️⃣ Apply the updated Deployment

Kubernetes replaces the old Pod with a new one.

→ Your `/story/text.txt` file is lost once (because it's a new Pod).
...
—which now lives **inside the emptyDir volume**.

### 3️⃣ Trigger the crash using `/error`
...
# 📌 Why “emptyDir” is NOT enough long-term

`emptyDir` solves:

✔️ Data loss on container restart
✖️ Data loss on Pod restart or replacement
