...
This lecture explains **how to use environment variables inside Kubernetes Pods**, and **why you would want to do that**—especially to avoid hard-coding values inside your application code.
...
* Configuration shouldn’t be inside code.
...
## 📦 **How Kubernetes provides environment variables**
...
## 🧱 **Where does it go in deployment.yaml?**
...
```bash
docker build -t /kub-data-demo:2 .
docker push /kub-data-demo:2
```

Then update the deployment to use the new tag:

```yaml
image: /kub-data-demo:2
```
...
* Kubernetes controls configuration → *cleaner, scalable setup*.
