...
Now the instructor shows how to put those values **into a separate Kubernetes resource called a ConfigMap**, which allows:
...
It’s similar to `.env` files in Docker, but stored inside the cluster.

---

# 📄 **1. Creating a ConfigMap (environment.yaml)**
...
# ⚙️ **2. Using a ConfigMap in a Deployment**
...
App behavior stays the same — only config handling changed.
...
Only modify the ConfigMap → all pods using it receive the updated values.

### ✔ Makes environment variables reusable
...
* Application code does NOT change — only how Kubernetes provides the value changes.
