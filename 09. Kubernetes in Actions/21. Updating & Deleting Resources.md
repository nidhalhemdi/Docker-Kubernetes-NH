...
# 1️⃣ Updating Resources (Declaratively)
...
* If replicas increase → Kubernetes creates more Pods.
* If replicas decrease → Kubernetes terminates Pods.
* If image changes → Kubernetes triggers a rolling update.
...
# 2️⃣ Example From the Lecture

### Scale up:

Edit replicas → 3
Apply → cluster creates new pods
`kubectl get pods` → now shows 3 pods.

### Scale down:

Change replicas → 1
Apply → 2 pods terminate.
...
Reload the browser → new version is running.

---

# 3️⃣ Why Declarative Updating Is Better
...
# 4️⃣ Deleting Resources (Declarative or Imperative)

You have two options to delete a resource:

---

## 🅰 Imperative Delete (by name)
...
## 🅱 Declarative Delete (recommended for YAML workflow)
...
* **Updating:** Edit YAML → apply → Kubernetes updates only what changed
