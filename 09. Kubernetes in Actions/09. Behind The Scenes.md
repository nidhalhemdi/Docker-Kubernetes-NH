# **kubectl: Behind the Scenes — Explained Simply**
...
a LOT happens behind the scenes. Understanding this is key to really “getting” Kubernetes.

---

# 1️⃣ **kubectl → Master Node (Control Plane)**
...
Minikube automatically configures `kubectl` to talk to your Minikube cluster, so you don’t need extra configuration.

---

# 2️⃣ **Master Node Processes the Request**
...
> “The bottom Worker Node has the most free resources, so I'll schedule the Pod there.”

---

# 3️⃣ **Worker Node Takes Over**
...
> “Please run this Pod using this container image.”
...
If you used a **local image**, the Worker Node cannot see it → *ImagePullError*.

This is exactly why you needed to push the image to Docker Hub.

---

# 5️⃣ **Result: The Pod Is Now Running**
...
* Does the Deployment’s desired state match the actual state?

Kubernetes always tries to ensure that real state = desired state.

---

# 🤝 Final Idea: Kubernetes Is a “Desired State Machine”

You tell Kubernetes what you **want**.

Kubernetes figures out **how to get there**.

kubectl → Deployment → Pod → Scheduler → Worker Node → kubelet → Container
