# **Kubernetes: Restarting Containers — Summary**

This lecture explains how Kubernetes automatically **detects and restarts failing containers** inside a Pod—**as long as the Pod was created through a Deployment** (or anything with a ReplicaSet).

---

# 1️⃣ **Triggering a Crash**

In the example:

* The Node.js app includes a `/error` route.
* Visiting this route **throws an error** → the Node server stops → the container also stops (because the container’s main process dies).

So when the container crashes, Kubernetes responds.

---

# 2️⃣ **What Happens After the Crash?**
...
▶ Kubernetes did **not** delete the Pod
▶ Kubernetes did **not** create a new Pod
▶ Kubernetes **restarted the container inside the same Pod**

---

# 3️⃣ **Restart Backoff Policy (Exponential Backoff)**
...
* Example timing: 10s → 20s → 40s → 80s…
...
…then the restart behavior depends on the restart policy, but Deployments guarantee `restartPolicy: Always`.
...
# 6️⃣ **Key Concept: Pod vs Container**
...
* A container crash ≠ a Pod crash → Pod remains, container is restarted.
