# **Exposing a Deployment with a Service — Summary**
...
…it’s easier to expose an existing Deployment with:
...
| ----------------------- | ---------------------------------------- | --------------------------- |
| **ClusterIP (default)** | Only reachable inside the cluster        | ❌ No                        |
| **NodePort**            | Opens a port on each Worker Node         | ✔ Yes                       |
| **LoadBalancer**        | Creates LoadBalancer in your cloud infra | ✔ Yes (best for production) |
...
# 3️⃣ Command to Expose the Deployment
...
# 5️⃣ Accessing the App in Minikube
...
→ the Service
→ which forwards to the Pod(s)
→ which runs the containerized Node app.

---

# 6️⃣ What You Achieved
...
These three objects (Pod → Deployment → Service) are the core of almost every Kubernetes application.
