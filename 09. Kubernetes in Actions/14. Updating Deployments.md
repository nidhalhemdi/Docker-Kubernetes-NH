...
This lecture explains how to update your application code inside a running Kubernetes Deployment — and how Kubernetes handles rolling updates behind the scenes.

---

# 1️⃣ **Step 1: Change Your Code → Rebuild Docker Image**
...
# 2️⃣ **Step 2: Push the Updated Image**
...
# 3️⃣ **Step 3: Use `kubectl set image` to Update the Deployment**
...
# 4️⃣ **Important: Kubernetes Only Pulls a New Image When the TAG Changes**

If you push a new image **with the same tag**, Kubernetes usually **does not re-download it**.

➡️ **Solution: Version your images.**
...
Now Kubernetes recognizes that the tag changed → pulls the new version → updates Pods.

---

# 5️⃣ **Check the Rollout Status**
...
You’ll see the new version of your app (after a few seconds).

### In the dashboard:

* Open **Pods** → select a Pod → see the **new image tag**.
...
* To update your app: **rebuild → push → set new image tag**.
