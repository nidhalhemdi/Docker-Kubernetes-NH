...
### Step 1 — Replace IP or env var with service DNS name
...
### Step 3 — Test your requests

Nothing changed in your code except the value passed to the env var.

Signup route → Now using DNS
Login route → Still using auto env variable (as demonstration)

Both still work → proves CoreDNS resolves service names.

---

# 📌 **Important Notes**

* **Works in any programming language** (DNS ≠ language-specific).
...
✔ No need to fetch IPs
✔ No manual updates
✔ Works automatically when pods restart
✔ Cleanest configuration
✔ Recommended in real production clusters
