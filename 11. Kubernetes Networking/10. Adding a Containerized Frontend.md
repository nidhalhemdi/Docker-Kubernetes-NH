...
Now we’ll test the real backend using an actual frontend UI.
...
→ It can reach the backend using the same IP used in Postman (minikube's exposed IP).

### **CORS (Cross-Origin Resource Sharing)**

Browsers block requests to another domain/IP unless headers allow it.
Postman doesn’t enforce this → works there, fails in browser.
...
# 4️⃣ **Run the Frontend Locally (not Kubernetes yet!)**
...
# 6️⃣ **Rebuild and Redeploy the Tasks API**
...
Because YAML didn’t change, you must delete the deployment:
...
# 7️⃣ **Fix Authorization Error in Frontend**
...
# 8️⃣ **Rebuild Frontend with Updated Code**
